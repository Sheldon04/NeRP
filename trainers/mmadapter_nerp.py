...
from trainers.mmadapter import MultiModalAdapter, load_clip_to_cpu, TextEncoder
...
class MultiModalAdapter_NeRP(MultiModalAdapter):
    ...
    def _get_text_prior(self, example_input):
        ...
        def _encode_one(s: str):
            ...
            tok_emb = self.model.adapter_learner.clip_model.token_embedding(tokens)       
            with torch.no_grad():
                t_zs = self.model.text_encoder(tok_emb, tokens)  # [1, D]
                if self.model.adapter_learner.text_adapter[0] is not None:
                    token_embedding = self.text_adapter[0].down(tok_emb)
                    ...
                    tok_emb = tok_emb + self.adapter_scale * token_embedding
                t = self.model.text_encoder(tok_emb, tokens, self.model.adapter_learner.text_adapter_func)
        ...
        if residual_prior:
            ...
            self._text_prior_logits = logit_scale * b_alpha_zs 
