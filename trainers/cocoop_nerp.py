...
from trainers.cocoop import CoCoOp, load_clip_to_cpu, TextEncoder
...
class CoCoOp_NeRP(CoCoOp):
    ...
    def _get_text_prior(self, example_input):
        ...
        _, _, text_features = self.model(example_input)   # [C,D]
        text_features = F.normalize(text_features.squeeze(0), dim=1)
        ...
        def _encode_one(s: str):
            ...
            token_prefix = tok_emb[:, :1, :].unsqueeze(0).expand(1, -1, -1, -1)
            token_suffix = tok_emb[:, 1 + self.cfg.TRAINER.COCOOP.N_CTX:, :].unsqueeze(0).expand(1, -1, -1, -1) 
            ctx_bnc = self.model.prompt_learner.ctx.unsqueeze(0).unsqueeze(1).expand(1, 1, -1, -1).to(self.device) 
            prompts = torch.cat([token_prefix, ctx_bnc, token_suffix], dim=2)   
            B, N, L, D = prompts.shape
            prompts_bn = prompts.reshape(B * N, L, D)    
            with torch.no_grad():
                t_zs = text_encoder_clip(tok_emb, tokens)  # [1, D]
                t = self.model.text_encoder(prompts_bn, tokens)
    ...
    def _get_image_prior(self, example_input):
        ...
        x = x.to(dtype=dtype)
        text_features_clip = self.model.prompt_learner.fixed_embeddings.half().to(device=device)
        text_features_clip = text_features_clip / text_features_clip.norm(dim=-1, keepdim=True)

        _, image_features, text_features = self.model(x)  # feats: [1,D] / [C,D]

        logit_scale = self.model.logit_scale.exp()

        prior_logits = logit_scale * (image_features @ text_features.squeeze(0).t())     # [1, C]
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            if batch_idx == 0:
                ...
                np_img = self._get_image_prior(example_input=input)
                self._get_text_prior(np_img)
