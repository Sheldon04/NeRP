...
def load_clip_to_cpu(cfg):
    ...
    model = clip.build_model_MultiModalAdapter(state_dict or model.state_dict())
    return model
...
class AdapterLearner(nn.Module):
    ...
    def _build_text_embedding(self, cfg, classnames, clip_model):
        ...
        with torch.no_grad():
            ...
            fixed_embeddings = self.clip_model.encode_text(tokenized_prompts)

        # fixed_embeddings = torch.cat(all_teacher_features, dim=1).mean(dim=1)

        self.register_buffer("fixed_embeddings", fixed_embeddings, persistent=False)
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        fixed_embeddings = self.adapter_learner.fixed_embeddings
        fixed_embeddings = F.normalize(fixed_embeddings, dim=-1)

        logit_scale = self.logit_scale.exp()
        logits = logit_scale * image_features @ text_features.t()

        return logits, image_features, text_features, fixed_embeddings
...
class MultiModalAdapter(TrainerX):
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            output, _, _, _ = self.model_inference(input)
