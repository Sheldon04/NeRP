...
from trainers.mmrl import MMRL, load_clip_to_cpu, TextEncoder_CLIP
...
class MMRL_NeRP(MMRL):
    ...
    def _get_text_prior(self, example_input):
        ...
        _, _, _, _, text_features, _ = self.model(example_input)   # [C,D]
        text_features = F.normalize(text_features, dim=1)

        clip_model = load_clip_to_cpu(self.cfg).float()

        text_encoder_clip = TextEncoder_CLIP(clip_model)

        def _encode_one(s: str):
            tokens  = clip.tokenize(s)                            # [1, L] on CPU
            tok_emb = clip_model.token_embedding(tokens).type(clip_model.dtype)  # [1, L, D] on CPU
            with torch.no_grad():
                t_zs = text_encoder_clip(tok_emb, tokens)  # [1, D]
                t = self.model.text_encoder(tok_emb.to(self.device), tokens.to(self.device), self.model.compound_rep_tokens_text_for_inference)
            return t_zs.squeeze(0).to(self.device), t.squeeze(0).to(self.device)
        ...
        if residual_prior:
            self._text_prior_logits = 100.0 * (b_alpha_zs - coe * b_alpha)  
        else: 
            self._text_prior_logits = 100.0 * b_alpha_zs       # [C]



    @torch.no_grad()
    def _get_image_prior(self, example_input, use_fusion=True):
        ...
        x = x.to(dtype=dtype)
        _, _, _, image_features, text_features, image_features_rep = self.model(x)  # feats: [1,D] / [C,D]

        text_features_clip = self.text_features_clip

        logits      = 100.0 * (image_features      @ text_features.t())     # [1, C]
        logits_rep  = 100.0 * (image_features_rep  @ text_features.t())     # [1, C]

        logits_zs      = 100.0 * (image_features      @ text_features_clip.t())     # [1, C]
        logits_rep_zs  = 100.0 * (image_features_rep  @ text_features_clip.t())     # [1, C]

        alpha = getattr(self, "alpha", None)
        if alpha is None:
            alpha = getattr(getattr(self.cfg, "TRAINER", type("obj", (), {})()).__dict__.get("MMRL", type("obj", (), {})()), "ALPHA", 0.5)
        alpha = float(alpha)

        logits_fusion_zs = alpha * logits_zs + (1.0 - alpha) * logits_rep_zs        
        logits_fusion = alpha * logits + (1.0 - alpha) * logits_rep      

        prior_logits_zs = logits_fusion_zs if use_fusion else logits_zs           
        prior_logits_zs = prior_logits_zs.squeeze(0)                            

        prior_logits = logits_fusion if use_fusion else logits            
    ...
    def test(self, split=None):
        ...
        if task == "B2N":
            use_fusion = (self.cfg.DATASET.SUBSAMPLE_CLASSES == "base")
        elif task == "FS":
            use_fusion = True
        elif task == "CD":
            use_fusion = (dataset == "ImageNet")
        else:
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            if batch_idx == 0:
                ...
                self._get_image_prior(example_input=input, use_fusion=use_fusion)
            ...
            logits, _, logits_fusion, _, text_features, _ = self.model(input)
            text_features = F.normalize(text_features, dim=1)

            output = logits_fusion if use_fusion else logits
